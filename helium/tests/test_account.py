...
class TestAccountCreationFailure:
    @pytest_asyncio.fixture(scope='class', autouse=True, params=(
        (
            lambda c: c(name='.', password='.', email='invalid_email'),
        ),
        (
            lambda c: c(name='.', password='.', email='duplicate@email.com'),
            lambda c: c(name='.', password='.', email='duplicate@email.com'),
        ),
        (
            lambda c: c(name='.', password='.', email='1@1.com', username='duplicate'),
            lambda c: c(name='.', password='.', email='2@2.com', username='duplicate'),
        )
    ), ids=('bad email', 'duplicate email', 'duplicate username'))
    async def response(self, request, create_account):
        resp = None

        for make_request in request.param:
            resp = await make_request(create_account)
