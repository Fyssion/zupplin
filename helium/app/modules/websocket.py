...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message: str):
        try:
            ...
            self.close(WebsocketError.INVALID_DATA, 'Invalid message received.')
            return

        try:
            opcode = WebsocketOpcode(int(data['opcode']))
        except ValueError:
            self.close(
                WebsocketError.INVALID_OPCODE, f'Invalid opcode received ({data["opcode"]}).'
            )
            return
...
def setup(app: Application):
    return (f'/websocket/connect', WebSocketHandler)
